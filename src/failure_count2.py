...
if __name__ == '__main__':
	...
	output_path = os.path.join(input_dir, 'failure_count2.mp4')
	...
	with mp_hands.Hands(
		...
		for f, f_mask, f_masked, f_inpainted in zip(frames_rgb, frames_mask, frames_rgb_masked, frames_inpainted):
			t = time.time()
			...
			if mp_result_orig and mp_result_orig.multi_hand_landmarks:
				...
				if tracking_started_orig and is_occluded:
			...
			if mp_result_inpainted and mp_result_inpainted.multi_hand_landmarks:
				...
				if tracking_started_inpainted and is_occluded:
...
Tracking failure count
    Original:  {failure_count_orig:4} [{failure_count_orig - failure_count_and}] / {frame_count}
    Inpainted: {failure_count_inpainted:4} [{failure_count_inpainted - failure_count_and}] / {frame_count}
