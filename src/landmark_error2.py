...
if __name__ == '__main__':
	...
	pole_mask = cv2.imread(r'resource/pole_mask_color.png')
	pole_mask = rgb2gray(pole_mask)
	pole_mask = binarize(pole_mask)
	...
	writer = cv2.VideoWriter(
		os.path.join(input_dir, 'lm_error2.mp4'),
	...
	with mp_hands.Hands(
		...
		for f_no_pole, f_pole, f_mask, f_masked, f_inpainted in zip(frames_no_pole, frames_pole, frames_mask, frames_pole_masked, frames_inpainted):
			t = time.time()

			is_occluded = (f_mask & pole_mask).any()
...
Landmark coords error
  Original:  {f'{landmark_coords_errors[0][-1]:> 6.2f}' if is_occluded and landmark_coords[1] is not None else f'{"N/A":>6}'} ave {np.mean(landmark_coords_errors[0]):> 6.2f}
  Inpainted: {f'{landmark_coords_errors[1][-1]:> 6.2f}' if is_occluded and landmark_coords[2] is not None else f'{"N/A":>6}'} ave {np.mean(landmark_coords_errors[1]):> 6.2f}
"""
			row = 30
	...
	np.savetxt(os.path.join(input_dir, 'lm_error_orig2.csv'), landmark_coords_errors[0])
	np.savetxt(os.path.join(input_dir, 'lm_error_inpainted2.csv'), landmark_coords_errors[1])
